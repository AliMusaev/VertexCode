...
namespace NxAssemblyReaderLib
{
    class BaseSectionBuilder
    {
        ...
        private VertexCommand CreateNotchCommand(Line[] lines, string partName)
        {
            ...
            MessageBox.Show($"Notch {midpoint}");
        ...
        private BaseAttributes GeteSizeAttributeFrom(ContourFlange flange)
        {
            ...
            topLines =  lines.Where(x => x.StartPoint.Z != x.EndPoint.Z && x.StartPoint.Y == x.EndPoint.Y).ToArray();
            ...
            topLines = lines.Where(x => x.StartPoint.Y != x.EndPoint.Y && x.StartPoint.Z == x.EndPoint.Z).ToArray();
